...
- 站点实现基于 Fumadocs + Next.js，但前端工程放在 `_site/`，不是顶层主结构。
- 顶层文档目录是唯一内容真源；`_site/content/docs/` 是构建前同步出的站内副本。
...
- `_site/next.config.mjs`：静态导出与 Pages 基础路径配置
- `_site/source.config.ts`：Fumadocs 内容源配置
- `_site/scripts/sync-content.mjs`：将顶层文档同步到站内内容目录
...
- 不要手改 `_site/content/docs/`，它是同步产物。
